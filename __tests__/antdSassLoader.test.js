...
describe('themeImporter', () => {
  it('produces an importer that allows importing compiled antd variables', async (done) => {
    ...
    const antDefaultLessPath = path.resolve(__dirname, '../node_modules/antd/lib/style/themes/default.less');
    const contents = await compileThemeVariables(themePath, antDefaultLessPath);
    sass.render({
